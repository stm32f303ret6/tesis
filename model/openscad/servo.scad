...
module servo()
{
   ...
   boxwidth = 20.0;
   boxheight = 41.0;
  //topheight, connectionheight, connectionlength, axisxposition, topclinderradius, wirelength, wirewidth, wireheight, wirezposition
  
		Servo_Hitec_Standard(false, boxlength, boxwidth, boxheight, 0, 28, 7.0, 30, 16, 6, 6, 3.5, 5);
}

// a module to make difference into another model, it just add some tolerance
module servo_for_difference()
{
   boxlength = 40.0 + 0.2;
   boxwidth = 20.0 + 0.2;
   boxheight = 41.0 + 0.2;
...
// 10 mm distance between screws
//translate([15,5,-14])cylinder(d=3,h=10,$fn=30);
//translate([15,-5,-14])cylinder(d=3,h=10,$fn=30);
module servo_for_mujoco(){
  rotate([90,90,0])
  servo();
  }

//servo();
  //rotate([180,0,270])servo_for_mujoco();