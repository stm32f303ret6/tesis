#!/usr/bin/env python3
"""
Visual explanation of Bézier gait parameters.

Uses the same ``DiagonalGaitController``-based trajectory generator as the
main simulation (via ``generate_gait_trajectory`` in test_bezier_gait.py),
which internally relies on the ``bezier`` and ``transitions`` libraries.
"""
...
# Allow running this file directly from tests/ by adding repo root to path
...
fig.suptitle('Bezier Gait Parameters Explained', fontsize=16, fontweight='bold')

# Default parameters (match GaitParameters usage)
default_params = {
    "num_points": 100,
    "step_height": 0.03,
    "step_length": 0.04,
...
def plot_trajectory_with_annotations(ax, params, title, highlight=None):
    ...
    # Plot trajectory
    ax.plot(swing[:, 0], swing[:, 2], 'r-', linewidth=3, label='Swing phase')
    ax.plot(stance[:, 0], stance[:, 2], 'g-', linewidth=3, label='Stance phase')

    # Mark start and end
    ax.plot(traj[0, 0], traj[0, 2], 'go', markersize=12, label='Start')
    ax.plot(traj[mid-1, 0], traj[mid-1, 2], 'ro', markersize=12, label='Swing end')
    ...
    if highlight is None or highlight == "step_length":
        ax.annotate(
            "",
            xy=(step_length / 2, body_height),
            xytext=(-step_length / 2, body_height),
    ...
    if highlight is None or highlight == "step_height":
        peak_height = max(body_height - step_height, 1e-4)
        mid_z = 0.5 * (body_height + peak_height)
        ax.annotate(
            "",
            xy=(0, body_height),
            xytext=(0, peak_height),
    ...
    if highlight is None or highlight == "body_height":
        ax.annotate(
            "",
            xy=(-step_length / 2 - 0.01, 0.0),
            xytext=(-step_length / 2 - 0.01, body_height),
            arrowprops=dict(arrowstyle="<->", color="purple", lw=2),
        )
        ax.text(
            -step_length / 2 - 0.015,
            body_height / 2,
            f"body_height\n{body_height*1000:.0f}mm\n(from base)",
            ha="right",
            va="center",
            fontsize=10,
            color="purple",
...
# Flip vertical axis so downward (larger z) appears lower on the plot
...
plot_trajectory_with_annotations(
    axes[0, 1],
    params_high_step,
    f'Increased step_height: {params_high_step["step_height"]*1000:.0f}mm\n(higher clearance)',
...
plot_trajectory_with_annotations(
    axes[1, 0],
    params_long_step,
    f'Increased step_length: {params_long_step["step_length"]*1000:.0f}mm\n(longer stride)',
    highlight="step_length",
)

# 4. Varying body_height
params_low_stance = default_params.copy()
params_low_stance["body_height"] = 0.06  # 60mm instead of 80mm
plot_trajectory_with_annotations(
    axes[1, 1],
    params_low_stance,
    f'Changed body_height: {params_low_stance["body_height"]*1000:.0f}mm\n(higher body position)',
    highlight="body_height",
)
...
3. step_height (default: 0.03m = 30mm)
   ...
   - Measured relative to body_height (downward from base)
   - Larger value = higher foot clearance over obstacles
   - Peak occurs near middle of swing phase

4. body_height (default: 0.08m = 80mm)
   - Height of foot when on ground (positive = below base)
   - This sets the robot's body height
   - Larger value = robot squats lower
   - Smaller value = robot stands taller
   - Must be within IK reachable range!

BEZIER SWING TRAJECTORY:

  The swing phase is generated by a cubic Bézier curve constructed by the
  ``DiagonalGaitController`` using the ``bezier`` library. Control points are
  derived from (step_length, step_height, body_height, swing_shape) and
  produce a smooth arc from back (stance) → lifted mid-swing → front (stance).

COORDINATE SYSTEM:
  X: Forward/backward (positive = forward)
  Y: Lateral/sideways (0 for basic gait)
  Z: Vertical (positive = down from base, 0 = base level)

TYPICAL VALUES FOR YOUR ROBOT:
  - Max reach: L1 + L2 = 0.045 + 0.06 = 0.105m (105mm)
  - Safe body_height range: 0.04 to 0.09m (40mm to 90mm)
