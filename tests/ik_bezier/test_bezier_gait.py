...
def test_gait_visualization():
    ...
    ax1.plot(valid_points[:, 0], valid_points[:, 1], valid_points[:, 2], 'b-', linewidth=2, label='Trayectoria')
    ...
    ax1.scatter(valid_points[-1, 0], valid_points[-1, 1], valid_points[-1, 2],
                c='red', s=100, marker='x', label='Fin')
    ax1.set_xlabel('X (m)')
    ax1.set_ylabel('Y (m)')
    ax1.set_zlabel('Z (m)')
    ax1.set_title('Trayectoria 3D del pie')
    ax1.legend()
    ax1.grid(True)

    # 2. Trajectory side view (XZ plane)
    ax2 = fig.add_subplot(132)
    ax2.plot(valid_points[:, 0], valid_points[:, 2], 'b-', linewidth=2, label='Trayectoria')
    ax2.scatter(valid_points[0, 0], valid_points[0, 2], c='green', s=100, marker='o', label='Inicio')

    # Mark swing/stance phases
    mid_point = len(valid_points) // 2
    ax2.plot(valid_points[:mid_point, 0], valid_points[:mid_point, 2],
             'r-', linewidth=3, alpha=0.5, label='Fase de vuelo')
    ax2.plot(valid_points[mid_point:, 0], valid_points[mid_point:, 2],
             'g-', linewidth=3, alpha=0.5, label='Fase de apoyo')
    ...
    ax3.plot(time_steps, np.degrees(joint_angles[:, 0]), label='Inclinación', linewidth=2)
    ax3.plot(time_steps, np.degrees(joint_angles[:, 1]), label='Hombro izquierdo', linewidth=2)
    ax3.plot(time_steps, np.degrees(joint_angles[:, 2]), label='Hombro derecho', linewidth=2)
    ax3.set_xlabel('Paso de tiempo')
    ax3.set_ylabel('Ángulo (grados)')
    ax3.set_title('Ángulos articulares durante el ciclo de marcha')
    ax3.grid(True)
    ax3.legend()
    ...
    ax_anim.set_title('Animación del movimiento de la pierna')
    ax_anim.grid(True)
    ax_anim.axis('equal')

    # Plot elements
    trajectory_line, = ax_anim.plot([], [], 'b--', alpha=0.3, label='Trayectoria')
    current_point, = ax_anim.plot([], [], 'ro', markersize=10, label='Objetivo actual')
    leg_left, = ax_anim.plot([], [], 'r-', linewidth=3, label='Brazo izquierdo')
    leg_right, = ax_anim.plot([], [], 'g-', linewidth=3, label='Brazo derecho')
