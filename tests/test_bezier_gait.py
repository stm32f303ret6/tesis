#!/usr/bin/env python3
"""
Test Bézier-based gait trajectories using the same libraries as the main controller.

Visualizes a single leg moving through a gait cycle generated by
``DiagonalGaitController`` (which internally uses the ``bezier`` and
``transitions`` libraries).
"""
...
# Leg parameters from ik.py / height_control.py
L1, L2 = 0.045, 0.06
IK_MODE = 2
BASE_DIST = 0.021
FORWARD_SIGN = -1.0  # Match height_control.py


def generate_gait_trajectory(
    num_points: int = 100,
    ...
    """
    Generate a gait trajectory for a single leg using DiagonalGaitController.

    Args:
        num_points: Number of points in the trajectory (one full cycle).
        step_height: Swing clearance relative to body_height (m).
        step_length: Forward/backward reach during the step (m).
        body_height: Nominal stance height (positive = downward from base, m).
        cycle_time: Duration of one full gait cycle (s).
        leg: Leg name ("FL", "FR", "RL", "RR").

    Returns:
        Array of shape (num_points, 3) with [x, y, z] foot positions in leg frame.
    """
    ...
    dt = params.cycle_time / float(num_points)
    trajectory = []

    for _ in range(num_points):
        leg_targets = controller.update(dt)
        target = leg_targets[leg].copy()
...
def test_gait_visualization():
    ...
    for i, target in enumerate(trajectory):
        ...
        result = solve_leg_ik_3dof(
            target_local, L1=L1, L2=L2, base_dist=BASE_DIST, mode=IK_MODE
        )
        if result:
...
def verify_fk_ik():
    ...
    for i, target in enumerate(test_targets):
        # Solve IK
        result = solve_leg_ik_3dof(
            target, L1=L1, L2=L2, base_dist=BASE_DIST, mode=IK_MODE
        )
        if result is None:
